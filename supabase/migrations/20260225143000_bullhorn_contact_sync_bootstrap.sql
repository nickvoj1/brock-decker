-- Bootstrap/repair migration for Bullhorn Contact Sync schema.
-- Safe to run on fresh DBs and on DBs where tables already exist.
-- Read-only mirroring: this does not write/delete anything in Bullhorn itself.
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      AND tablename = 'bullhorn_sync_jobs'
      AND policyname = 'Service role only for bullhorn_sync_jobs'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_trigger
    WHERE tgname = 'update_bullhorn_sync_jobs_updated_at'
  ) THEN
...
-- Ensure existing environments allow 5-contact test runs
ALTER TABLE public.bullhorn_sync_jobs
  DROP CONSTRAINT IF EXISTS bullhorn_sync_jobs_batch_size_check;

ALTER TABLE public.bullhorn_sync_jobs
  ADD CONSTRAINT bullhorn_sync_jobs_batch_size_check
  CHECK (batch_size >= 1 AND batch_size <= 2000);
...
BEGIN
  IF NOT EXISTS (
    ...
    FROM pg_policies
    WHERE schemaname = 'public'
      AND tablename = 'bullhorn_client_contacts_mirror'
      AND policyname = 'Service role only for bullhorn_client_contacts_mirror'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_trigger
    WHERE tgname = 'update_bullhorn_client_contacts_mirror_updated_at'
  ) THEN
